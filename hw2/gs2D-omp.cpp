...
// g++ -std=c++11 -O3 -o gs2D-omp gs2D-omp.cpp
...
long calcRowMajorIndex(long i, long j, long columnSize){
  long idx = (j + ((i - 1) * columnSize)) - 1;
  return idx;
}

/*
double* calculate_next_u_coloring_round(long idx, double* arr_u_k_plus_1, long n, long range, double h, double* arr_u_k, double* arr_f){
}
*/

double* calculate_next_u(long n, long range, double h, double* arr_u_k, double* arr_f){
  
  // use the Gauss-Seidel method: red-black coloring
  // calculate arr_u_1
  double *arr_u_k_plus_1 = new double[range];
  
  // calculate all red points
  // update to k(for calc black) and k+1
  // printf("n=%ld\n", n);
  // printf("Red stage:\n");
  #pragma omp parallel
  {
    ...
    #pragma omp for
  for(long idx = 0; idx < range; idx+=2){
    
    long j = ((idx + 1) % n == 0)? n: (idx + 1) % n;
    long i = ceil((idx + 1.0) / n);
    // printf("arr_u_k[%ld]: i=%ld, j=%ld\n", idx, i, j);
    ...
    double u_k_plus_1_i_j = (std::pow(h, 2) * f_i_j + u_k_im1_j + u_k_i_jm1 + u_k_ip1_j + u_k_i_jp1) / 4;
    
    // update k + 1 and update k(for black updating round)
    arr_u_k_plus_1[idx_i_j] = u_k_plus_1_i_j;
    arr_u_k[idx_i_j] = u_k_plus_1_i_j;
    // printf("i=%ld, j=%ld, u(k+1)_i_j=%10f\n", i, j, arr_u_k_plus_1[idx_i_j]);
    
  }
  
  // calculate all black points
  // update to k+1
  // printf("Black stage:\n");
    #pragma omp for
  for(long idx = 1; idx < range; idx+=2){
